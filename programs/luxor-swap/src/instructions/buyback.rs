...
pub fn buyback(ctx: Context<Buyback>) -> Result<()> {
    ...
    // --- Move SOL from stake PDA → admin token_0_account (WSOL ATA), then sync_native ---
    let ix = transfer(
    ...
    // Stake PDA signs with its seeds; this is a system transfer of lamports.
    ...
    let fee_treasury = (reward_available_to_buyback as u128)
    ...
    // --- Update stake-level accounting before executing external swap ---
    stake_info.last_tracked_sol_balance = ctx.accounts.stake_pda.lamports();
    ...
    emit!(BuybackExecuted {
        sol_amount: reward_available_to_buyback,
        lxr_bought,
