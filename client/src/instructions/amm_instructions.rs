...
use anyhow::Ok;
use anyhow::Result;
use luxor_stake::luxor_pool_state;
use solana_sdk::{instruction::Instruction, pubkey::Pubkey, system_program};

use luxor_stake::accounts as raydium_cp_accounts;
use luxor_stake::instruction as raydium_cp_instructions;
use luxor_stake::raydium_cpmm;
use luxor_stake::vault_and_lp_mint_auth;
use std::rc::Rc;
...
pub fn initialise_configs_instr(
    ...
    let program = client.program(config.luxor_stake_program)?;
    ...
    let instructions = program
        ...
        .accounts(raydium_cp_accounts::InitialiseConfigs {
            ...
            luxor_mint: luxor_stake::luxor_mint::id(),
...
pub fn purchase_instr(
    ...
    let ixs = program
        ...
        .accounts(raydium_cp_accounts::Purchase {
            ...
            token_1_vault: get_raydium_vault(&raydium_cpmm::id(), &luxor_stake::luxor_mint::id()),
        })
...
pub fn buyback_instr(config: &ClientConfig) -> anyhow::Result<Vec<Instruction>> {
    ...
    let ixs = program
        ...
        .accounts(raydium_cp_accounts::Buyback {
            ...
            vault_1_mint: luxor_stake::luxor_mint::id(),
...
pub fn emergency_withdraw_instr(
    ...
    let ixs = program
        ...
        .args(raydium_cp_instructions::EmergencyWithdraw { param })
        .instructions()?; // build the instruction(s)
